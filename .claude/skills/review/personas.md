...
**ISMS-P / 개인정보보호**: PROJECT_CONTEXT 에 주입된 `.claude/rules/security-compliance.md` 본문이 있으면 그 체크리스트를 **리뷰 기준으로 반드시 확인**합니다 (항목·세부 기준의 **정본은 그 규칙 파일**입니다 — 아래는 요약).
개인정보를 노출·처리하는 변경에 적용하고(처리방침 표시·원문 조회 권한 제한·서버/API 마스킹), 노출 지점이 없으면 "해당 없음" 으로 명시합니다.
세부 기준(마스킹 포맷·권한 체계 등)은 프로젝트 정책 우선이며, 정책이 없으면 위반 단정 대신 "확인 필요" 로 표기합니다.
이 규칙 위반 후보는 **정책성 항목이므로 자동 반영 대상이 아니며, 사람에게 escalate**합니다.
