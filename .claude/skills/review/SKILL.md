---
name: review
description: 시니어/보안/아키텍트/QA 4명 페르소나가 Agent 툴로 병렬 호출되어 현재 변경 diff를 리뷰 → 합성된 리뷰 코멘트를 gh pr comment로 PR에 등록. TRIGGER when 사용자가 /review 호출하거나, /pr 워크플로우에서 코드 리뷰 단계 진입 시.
---
...
- 이 브랜치 PR의 base 브랜치 (있으면): !`gh pr list --state all --head "$(git branch --show-current)" --json baseRefName 2>/dev/null | grep -o '"baseRefName":"[^"]*"' | head -1 | sed 's/.*:"//;s/"$//' || echo "(PR 없음/gh 미설치)"`
- Branch 변경 요약 vs base: !`TGT=$(gh pr list --state all --head "$(git branch --show-current)" --json baseRefName 2>/dev/null | grep -o '"baseRefName":"[^"]*"' | head -1 | sed 's/.*:"//;s/"$//'); TGT=${TGT:-dev}; echo "(base=origin/$TGT — PR 있으면 그 base, 없으면 기본 dev; 실제 base는 아래 우선순위로 확정)"; git diff "origin/$TGT...HEAD" --stat 2>/dev/null || echo "(origin/$TGT 없음 — project.config.md의 대상 브랜치 확인)"`

## 호출 모드 (PR 코멘트 등록 여부)

`$ARGUMENTS`에 `--no-note`(또는 `internal`)가 포함되면 **internal 모드**로 동작합니다:

- **Step 6 / Step 7 의 `gh pr comment` 등록을 건너뜁니다.** 합성된 리뷰 결과를 호출자에게 **반환만** 합니다.
- 자동 호출자(예: `/review-converge`)가 매 라운드 `/review`를 돌릴 때 사용합니다 — 라운드마다 중간 코멘트가 PR에 쌓이는 것을 막기 위함입니다. 최종 수렴 결과 코멘트는 호출자가 한 번만 등록합니다.
- `--no-note`가 없으면 기존대로 **Step 6에서 PR 코멘트를 등록**합니다 (사람이 직접 `/review`를 부르는 일반 케이스).
...
3. **Branch diff vs 대상(base) 브랜치** — 위 둘이 모두 비어 있으면 (커밋 완료 → PR 검토 시나리오) `origin/<대상브랜치>...HEAD`를 사용. **대상 브랜치는 아래 "대상 브랜치 결정"으로 정합니다.**

세 경우 모두 비어 있으면 사용자에게 안내하고 종료.

### 대상 브랜치 결정 (3순위에서 base 확정)

리뷰 비교 대상은 `main` 고정이 아니라 **이 변경이 들어갈 PR의 base**입니다(개발 반영→`dev`, 검증 반영→`staging`, 상용 반영→`product`; 브랜치명은 프로젝트마다 다름). 다음 순으로 확정합니다:

1. **이 브랜치 PR의 `baseRefName`** — 현재 브랜치에 PR이 이미 있으면 그 PR이 어디로 향하는지를 직접 읽어 base로 씁니다(개발/검증/상용을 PR 자신이 알고 있어 가장 정확). 자동 수집의 "이 브랜치 PR의 base 브랜치"가 그 값입니다. 조회 명령(`jq` 없이도 동작, 기본 `pr list`가 열린 PR만 보므로 `--state all` 권장):

   ```bash
   gh pr list --state all --head "$(git branch --show-current)" --json baseRefName \
     | grep -o '"baseRefName":"[^"]*"' | head -1 | sed 's/.*:"//;s/"$//'
   ```

   여러 PR이 잡히면 첫 번째(보통 최신)를 씁니다. 결과가 비면 다음 순위로.

2. **PR이 아직 없으면** `.claude/project.config.md`의 `DEFAULT_TARGET_BRANCH`(기본 `dev`) — 가장 흔한 개발 반영 케이스.
3. **검증/상용 반영처럼 base가 `DEFAULT_TARGET_BRANCH`가 아닌데 PR도 아직 없으면** — 자동 판별 불가이므로 `AskUserQuestion`으로 대상 브랜치(`DEFAULT_TARGET_BRANCH`/`STAGING_BRANCH`/`PRODUCTION_BRANCH`)를 한 번 확인합니다. (또는 호출 시 `$ARGUMENTS`로 범위 명시)

> 📌 **자동 호출자(예: `/pr`)에게 권장**: `/pr`은 1단계에서 반영 대상(개발/검증/상용)을 이미 받으므로, 항상 `$ARGUMENTS`로 그 대상 기준 범위를 명시 패스하세요 (예: `/review origin/staging...HEAD`). 명시 패스하면 위 3순위 자동 판별을 거치지 않아 가장 안전하고, 우연히 남은 working/staged dirty change가 잘못 잡히는 것도 막습니다.
...
PR 번호 조회 실패 시 사용자에게 확인. gh 미설치 시 본문을 세션에 출력하고 수동 등록 안내.
...
**해결 코멘트를 같은 PR에 등록**한다. 재리뷰(`/review` 재실행)를 돌리지 않는 경우에도 등록한다.

> ⚠️ **internal 모드(`--no-note`)면 이 단계도 건너뜁니다.** 수렴 결과·해결 내역은 호출자(`/review-converge`)가 최종 코멘트에 묶어 한 번만 등록합니다.

- **시점**: Blocker 수정 커밋이 push되어 PR diff에 반영된 직후.
- **내용**: Blocker마다 **조치 / 사유(해당 시) / 반영 커밋 해시**를 1~3줄로. 미반영 항목은 "후속"으로 명시.
- Non-blocker를 함께 고쳤으면 짧게 덧붙인다.
- gh 미설치 시 본문을 세션에 출력하고 수동 등록 안내.
...
> ⚠️ **GitHub 코멘트 넘버링 주의**: 단순 항목 번호에 `#`을 붙이지 마세요(`#1`·`#2`). GitHub은 `#숫자`를 **이슈/PR 번호 링크**로 자동 변환하므로 엉뚱한 이슈가 링크됩니다. 항목 번호는 `Blocker 1`·`1.`처럼 `#` 없이 씁니다. `#`는 실제로 이슈/PR을 가리킬 때만(`Closes #5`) 사용합니다.
