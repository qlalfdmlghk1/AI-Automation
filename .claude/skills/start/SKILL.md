...
allowed-tools: Bash(git:*), Bash(glab:*), Bash(which:*), Read, Write, Edit, AskUserQuestion, Skill(plan-review), Skill(plan-review *), mcp__atlassian__getAccessibleAtlassianResources, mcp__atlassian__getVisibleJiraProjects, mcp__atlassian__getJiraIssue, mcp__atlassian__createJiraIssue, mcp__atlassian__editJiraIssue
---
...
plan.md를 작성했으면, 외부 리소스(Jira·GitLab·branch)를 만들기 **전에** 기획의 빈틈을 한 번 검수합니다. 기획서에 정책 모순·미정의 케이스·흐름 누락이 남은 채로 티켓을 끊으면, 그 빈틈이 개발 중에 재작업·핑퐁으로 돌아오기 때문입니다.
...
3. **검수 결과로 게이트 판정:**
   ...
   - **🚫 개발 착수 불가 ≥ 1건** → **여기서 중단합니다.** Stage 2~4(Jira/GitLab/branch 생성)로 진행하지 않습니다.
...
"plan.md 먼저 수정"을 선택하면 **여기서 즉시 종료**합니다 (Stage 2 진입 금지). 이 시점에 plan.md 메타 표의 `Jira`/`GitLab Issue`/`Branch` 셀은 아직 비어 있는 상태이므로, 사용자가 plan.md를 직접 손본 뒤 동일 인자로 `/start`를 다시 호출하면 1-2의 분기 규칙에 따라 자동으로 **재개 모드**(후속 이슈 모드 아님)로 진입하여 1-3을 스킵하고 Stage 1.5(기획 검수)부터 다시 흐릅니다.
