---
name: start
description: 작업 시작 — plan.md 작성 → Jira 처리 → GitHub 이슈 → 브랜치 생성 → progress.md 생성
argument-hint: '[Jira 티켓 키] (선택, 없으면 Stage 0에서 직접 입력)'
allowed-tools: Bash(git:*), Bash(gh:*), Bash(which:*), Read, Write, Edit, AskUserQuestion, Skill(plan-review), Skill(plan-review *), mcp__atlassian__getAccessibleAtlassianResources, mcp__atlassian__getVisibleJiraProjects, mcp__atlassian__getJiraIssue, mcp__atlassian__createJiraIssue, mcp__atlassian__editJiraIssue
---
...
- gh_username: (Stage 3 진입 시 `gh api user` JSON 응답에서 `.login` 추출)

---

당신은 작업 시작 도우미입니다. plan.md 작성부터 progress.md 생성까지 한 번에 처리합니다.

핵심 원칙: Jira/GitHub을 먼저 부르면 body가 빈약해서 plan.md도 부실해집니다.
그래서 plan.md를 가장 먼저 작성하고, 그 내용을 Jira/GitHub 티켓에 채우는 방향으로 동작합니다.
...
**중요**: 사용자가 입력한 **원본 한글 제목은 반드시 보존**합니다 — Jira summary, GitHub 이슈 제목, plan.md 본문 등에 그대로 사용. kebab-case는 폴더명·브랜치명 전용입니다.
...
- **`Jira` 셀이 비어 있음** (Stage 1.5 기획 검수에서 개발 착수 불가로 중단 / Stage 1.6에서 "plan.md 먼저 수정" 후 재진입 / 또는 Stage 2~4 도중 실패로 중단된 케이스) → **재개 모드**로 전환
  - 1-3(목적/범위/접근 방식/참고 자료 입력) 스킵
  - 기존 plan.md 본문을 그대로 사용하고 Stage 1.5(기획 검수)부터 다시 진행
  - 일반화: plan.md 메타 표는 "Jira → GitHub Issue → Branch" 순으로 채워지므로, **비어 있는 첫 셀에 해당하는 Stage부터 재개**
...
**Jira / GitHub Issue / Branch 항목은 빈 상태로 둡니다** (Stage 2~4에서 자동 갱신).
...
| GitHub Issue | (Stage 3 완료 후 자동 주입)           |
...
### Stage 1.5: 기획 검수 게이트 (`/plan-review` — 표준 권장)

plan.md를 작성했으면, 외부 리소스(Jira·GitHub·branch)를 만들기 **전에** 기획의 빈틈을 한 번 검수합니다. 기획서에 정책 모순·미정의 케이스·흐름 누락이 남은 채로 티켓을 끊으면, 그 빈틈이 개발 중에 재작업·핑퐁으로 돌아오기 때문입니다.

이 게이트는 **표준 권장**입니다(강제는 아님). 다만 **개발 착수 불가 항목이 발견되면 외부 리소스 생성을 막는 방향**으로 동작합니다.

**절차:**

1. 평문으로 한 줄 안내 후 `AskUserQuestion` 카드 1회로 실행 여부를 확인합니다:

   - question: `외부 리소스를 만들기 전에 기획 검수(/plan-review)를 돌릴까요? (권장)`
   - options:
     - `{ label: "기획 검수 실행", description: "정책 모순·미정의 케이스·흐름 누락을 질문 목록으로 정리 (권장)" }`
     - `{ label: "건너뛰기", description: "검수 없이 Stage 1.6 확인 게이트로" }`

2. **"기획 검수 실행"** 선택 시 → `/plan-review`를 **기능 단위 모드**로 호출합니다 (대상: 방금 작성한 `docs/features/{기능명}/plan.md` + 1-3에서 받은 참고 자료 링크).

   - `/plan-review`는 정책을 **확정하지 않습니다.** 빈틈을 질문 목록으로 정리하고, 각 항목에 심각도(🚫 개발 착수 불가 / ⚠️ 반드시 확인 / 💡 참고)를 붙여 `docs/features/{기능명}/planning-review.md`에 저장합니다.
   - **AI가 빈틈을 임의 정책으로 채워 plan.md를 고치지 않습니다.** 정책 판단은 사용자/기획자가 합니다.

3. **검수 결과로 게이트 판정:**

   - **🚫 개발 착수 불가 ≥ 1건** → **여기서 중단합니다.** Stage 2~4(Jira/GitHub/branch 생성)로 진행하지 않습니다.
     - 사용자에게 `planning-review.md`의 최종 확인 질문 목록을 안내하고, **사용자가 답변·결정한 뒤 plan.md를 수정**하도록 요청합니다.
     - 사용자가 수정 후 동일 인자로 `/start`를 다시 호출하면, plan.md 메타 표의 `Jira` 셀이 비어 있으므로 1-2 분기 규칙에 따라 **재개 모드**로 진입해 Stage 1.5(기획 검수)부터 다시 흐릅니다. 재검수에서 개발 착수 불가가 0건이 되면 다음으로 진행됩니다.
   - **🚫 개발 착수 불가 0건** → ⚠️/💡 항목을 요약 안내하고 **Stage 1.6으로 진행**합니다 (⚠️ 항목은 착수 전 답변 권장이나 차단하지는 않음).

4. **"건너뛰기"** 선택 시 → 검수 없이 Stage 1.6으로 진행합니다. (검수를 생략했음을 짧게 남깁니다.)

---

### Stage 1.6: 외부 리소스 생성 전 확인 게이트 (필수)

Stage 2~4는 Jira 티켓·GitHub 이슈를 **실제로 생성**하고 브랜치를 **체크아웃**합니다. 한 번 만들어진 이슈는 알림이 이미 나가 되돌리기 비용이 커요. 그래서 plan.md 작성 직후 사용자 확인을 한 번 받습니다.
...
- question: `이대로 Jira·GitHub 이슈를 생성하고 브랜치를 만들까요?`
- options:
  - `{ label: "이대로 진행", description: "Stage 2~4 자동 실행" }`
  - `{ label: "plan.md 먼저 수정", description: "수정 완료 후 다시 /start 호출" }`

"plan.md 먼저 수정"을 선택하면 **여기서 즉시 종료**합니다 (Stage 2 진입 금지). 이 시점에 plan.md 메타 표의 `Jira`/`GitHub Issue`/`Branch` 셀은 아직 비어 있는 상태이므로, 사용자가 plan.md를 직접 손본 뒤 동일 인자로 `/start`를 다시 호출하면 1-2의 분기 규칙에 따라 자동으로 **재개 모드**(후속 이슈 모드 아님)로 진입하여 1-3을 스킵하고 Stage 1.5(기획 검수)부터 다시 흐릅니다.
...
> `gh`는 GitHub 전용 CLI라 Jira REST와 무관 — `gh api /rest/api/3/...` 같은 호출 금지.
> 사용 툴:
>
...
### Stage 3: GitHub 이슈 생성

plan.md + Jira description 기반으로 GitHub 이슈를 생성합니다.
...
- 업데이트 모드(Stage 0에서 확정된 경우)에는 Stage 0에서 보관한 summary가 1-2 작업 제목 기본값으로 쓰이므로 이를 사용 — 단, 1-2에서 "직접 수정"으로 제목을 바꿨으면 그 입력값을 우선합니다. summary 앞에 `[공통]` 등 다른 prefix가 붙어 있으면 제거 후 `[Type]`만 붙임.
  - 예: Jira summary `[공통] 설정 화면 개발자 테스트 및 수정` → GitHub 제목 `[Refactor] 설정 화면 개발자 테스트 및 수정`

Type 매핑 (제목 prefix + GitHub 라벨):

| Stage 1 Type | 제목 prefix  | GitHub `--label` |
| ------------ | ------------ | ---------------- |
| Feat         | `[Feat]`     | `feat`           |
| Fix          | `[Fix]`      | `fix`            |
...
gh가 설치된 경우 `gh issue create`로 직접 생성합니다. **`--assignee`와 `--label`은 필수**입니다:
...
- `--assignee`: `gh api user`를 실행하여 JSON 응답의 `login` 필드를 추출해서 사용 (작성자 본인). 같은 세션 내에서는 캐시.
- `--label`: 위 Type 매핑 표의 GitHub `--label` 컬럼 값.
...
**gh 미설치/미인증 시 (fallback):**

1. 위 이슈 본문을 `docs/features/{기능명}/github-issue-draft.md`로 저장하고, GitHub 웹에서 수동 생성하도록 안내합니다.
2. 사용자가 생성한 **이슈 번호를 입력**받아 plan.md 메타 표를 갱신한 뒤 Stage 4로 계속합니다.
   (이슈 번호는 Stage 4 브랜치명에 필요하므로, 입력 전에는 다음 단계로 진행하지 않습니다.)

생성 후 plan.md의 메타 표 `GitHub Issue` 항목을 갱신합니다.

---

### Stage 3.5: Jira 티켓에 GitHub 이슈 링크 업데이트

GitHub 이슈 생성 완료 후, Jira 티켓 description 하단에 GitHub 이슈 URL을 append 합니다. `editJiraIssue`의 `description` 필드는 **전체 치환**이므로, append를 위해 다음 절차를 반드시 따릅니다:

1. **재조회** — `getJiraIssue`로 최신 description과 포맷을 확인 (`contentFormat: "markdown"`을 명시해 평문 markdown으로 받음).
2. **중복 검사** — 기존 description에 `## 🔗 GitHub 이슈` 섹션이 이미 있는지 확인.
...
📌 Jira:        {JIRA_SITE_URL}/browse/{Jira키}
🔗 GitHub 이슈:  {GITHUB_REPO_URL}/issues/{이슈번호}
