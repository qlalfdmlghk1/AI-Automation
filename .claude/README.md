...
1. 팀 표준 플러그인을 설치합니다 (한 번만):
   ```bash
   claude plugin marketplace add https://dep.dawin.tv/adplatforms/ux-ai-automation.git
   claude plugin install ux-team-standard@ux-ai-automation
   ```
   > 🌿 **배포 채널**: 위 명령은 **안정 채널**(default branch `main` = 릴리스)입니다. 릴리스 전 dev를 미리 써보려면 `…ux-ai-automation.git#dev`로 브랜치를 pin하고 `/plugin marketplace update` + `/reload-plugins`로 갱신하세요. 저장소 default branch는 `main`으로 두는 걸 권장합니다(무-지정 설치가 안정 버전을 받도록). 상세는 루트 [README](../README.md#배포-채널--안정main--실험dev).
2. 대상 프로젝트 루트에서 `/team-init`을 실행합니다 — 스택을 감지해 `rules/`·`CLAUDE.md`·`settings.json`·`hooks/`·`project.config.md`를 배치하고 자동 도출 값을 채웁니다.
3. `.claude/project.config.md`에서 Jira, GitLab, Confluence, 기본 브랜치, reviewer 값을 확인·입력합니다.
